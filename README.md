...
* **[2024-03-23]**: 彻底修复 `openWindow` 时因同时指定 `displayId` (坐标) 和 `state: 'fullscreen'` 导致的 Chrome API 底层报错 `Invalid value for state` 的问题，通过创建后延迟全屏的方式绕过 Chrome 限制。进一步优化了全屏时的延迟逻辑，防止 Chrome 在过快更新状态时将窗口拉回主屏。
